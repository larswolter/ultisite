...
Template.registerHelper("pathFor", function (params, params2, params3) {
  ...
  if (params.hash) { return FlowRouter.path(params.hash.route, params.hash); } else if (typeof (params) === "string") { return FlowRouter.path(params, (params2 || {}).hash); }
  console.log("pathFor Error:", params);
});
Template.registerHelper("subsReady", function (type) {
  const sub = UltiSite[`${type}Ready`].get();
...
Template.registerHelper("joinNice", function (array) {
  ...
  const text = ` und ${_.clone(array).pop()}`;
...
Template.registerHelper("colorState", function (state) {
  if (state === 100) { return "success"; } else if (state >= 50) { return "warning"; } else if (state >= 10) { return "default"; }
  return "muted";
});
...
Template.registerHelper("formatFileSize", function (size) {
  if (size > 1024 * 1024 * 1024 * 1024) { return `${(size / (1024 * 1024 * 1024 * 1024)).toFixed(2)}T`; } else if (size > 1024 * 1024 * 1024) { return `${(size / (1024 * 1024 * 1024)).toFixed(2)}G`; } else if (size > 1024 * 1024) { return `${(size / (1024 * 1024)).toFixed(2)}M`; } else if (size > 1024) { return `${(size / (1024)).toFixed(2)}K`; }
  return `${size}`;
});
...
Template.registerHelper("translate", function (term) {
