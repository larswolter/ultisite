...
Template.registerHelper('sitePageHeader', function () {
  ...
  switch (name) {
    case 'tournaments':
      return { name: 'Turniere', link: '/' };
    case 'practices':
      return { name: 'Trainingszeiten', link: '/' };
    case 'admin':
      return { name: 'Administrierung', link: '/' };
    case 'blog':
      return { name: 'Artikel', link: '/' };
    case 'tweets':
      return { name: 'Blabla', link: '/' };
    case 'wikipage':
      var wiki = UltiSite.WikiPages.findOne({
        $or: [{ _id: FlowRouter.getParam('_id') }, { name: FlowRouter.getParam('_id') }],
      });
      if (wiki) {
