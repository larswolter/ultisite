...
Template.wikipage.events({
  ...
  'click .action-add-discussion': function (e, t) {
    ...
    UltiSite.getHTMLTextDialog({ content: '', header: 'Diskussionsbeitrag hinzufügen' }, (text) => {
      Meteor.call(
        'addWikiPageDiscussion',
        t.activePage.get()._id,
        text,
...
Template.wikipage.helpers({
  ...
  discussions() {
    return UltiSite.WikiPageDiscussions.find(
      { pageId: (Template.instance().activePage.get() || {})._id },
      { sort: { date: -1 } }
    ).fetch();
  },
  lastDiscussionDate() {
    return (
      UltiSite.WikiPageDiscussions.findOne(
        { pageId: (Template.instance().activePage.get() || {})._id },
        { sort: { date: -1 } }
      ) || {}
    ).date;
  ...
  histories() {
    return UltiSite.ContentVersions.find(
      { associated: (Template.instance().activePage.get() || {})._id },
      { sort: { date: -1 } }
    ).fetch();
  },
  wikiPages() {
