var oldWiki =
  '][===Willkommen beim Wiki = Beschreibung===\n' +
  '\n' +
  'Links:\n' +
...
const wikiPageTranslateOld = function (content) {
  var result = content || '';
  ...
  result = result.replace(
    /\[\[bild\:(http[^\]\|]+)\|(http:\/\/[^\]\|\s]*)\|([^\]\s]*)\]\]/g,
    "<a href='$2'><img src='$1' class='pull-$3' ></a>"
  );
  ...
  result = result.replace(
    /\[\[bild\:([^\]\|]+)\|(http:\/\/[^\]\|\s]*)\|([^\]\s]*)\]\]/g,
    "<a href='$2'><img data-name='$1' class='pull-$3' src='/public/missing.png' ></a>"
  );
  result = result.replace(
    /\[\[bild\:([^\]\|]+)\|(http:\/\/[^\]\|\s]*)\]\]/g,
    "<a href='$2'><img data-name='$1' src='/public/missing.png' ></a>"
  );
  result = result.replace(
    /\[\[bild\:([^\]\|]+)\|([^\]\|\s]*)\]\]/g,
    "<img data-name='$1' src='/public/missing.png' class='pull-$3' >"
  );
  result = result.replace(/\[\[bild\:([^\]]+)\]\]/g, "<img data-name='$1' src='/public/missing.png'>");
  result = result.replace(
    /\[\[dokument\:([^\]]+)\]\]/g,
    "<span class='fa fa-file'></span><a data-name='$1' href='#'>Missing</a>"
  );
  ...
  lines = lines.map(function (line, idx) {
    var rueck = line;
    if (line.indexOf('][') === 0) rueck = '<center>' + line.substr(2) + '</center>';
    if (line.indexOf('***') === 0 || line.indexOf('###') === 0) rueck = '<li>' + line.substr(3) + '</li>';
    else if (line.indexOf('**') === 0 || line.indexOf('##') === 0) rueck = '<li>' + line.substr(2) + '</li>';
    else if (line.indexOf('*') === 0 || line.indexOf('#') === 0) rueck = '<li>' + line.substr(1) + '</li>';

    if (line.indexOf('*') === 0 && (idx === 0 || lines[idx - 1].indexOf('*') !== 0)) rueck = '<ul>' + rueck;
    if (line.indexOf('*') !== 0 && idx !== 0 && lines[idx - 1].indexOf('*') === 0) rueck = '</ul>' + rueck;
    if (line.indexOf('#') === 0 && (idx === 0 || lines[idx - 1].indexOf('#') !== 0)) rueck = '<ol>' + rueck;
    if (line.indexOf('#') !== 0 && idx !== 0 && lines[idx - 1].indexOf('#') === 0) rueck = '</ol>' + rueck;

    if (line.indexOf(' ') === 0 && (idx === 0 || lines[idx - 1].indexOf(' ') !== 0)) rueck = '<pre>' + rueck;
    if (line.indexOf(' ') !== 0 && idx !== 0 && lines[idx - 1].indexOf(' ') === 0) rueck = '</pre>' + rueck;

    if (line.indexOf('<th>') === 0 || line.indexOf('<td>') === 0) rueck = '<tr>' + rueck + '</tr>';

    if (line.indexOf('<t') === 0 && (idx === 0 || lines[idx - 1].indexOf('<t') !== 0))
      rueck = "<table class='table table-hover'>" + rueck;
    if (line.indexOf('<t') !== 0 && idx !== 0 && lines[idx - 1].indexOf('<t') === 0) rueck = '</table>' + rueck;
