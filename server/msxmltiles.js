WebApp.connectHandlers.use("/msxmltiles", function(req, res, next) {
    ...
    if(content==='images') {
        let img = UltiSite.Images.findOne({tags:'Teamfoto'},{sort:{created:-1},fields:{base64:0}});
        if(img) {
    ...
    res.end(UltiSite.renderMailTemplate(layout,template, data));
