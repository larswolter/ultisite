...
Meteor.startup(function () {
  const job = new CronJob('0 9 15 * * *', Meteor.bindEnvironment(() => {
    if (!UltiSite.settings().mailingListConfigs) { return; }
            ...
            msg.on('end', Meteor.bindEnvironment(function (attrs) {
              ...
              if (user) {
                ...
                structure.forEach((part) => {
                  if (part.partID) { parts.push(part); } else if (Array.isArray(part)) {
                    part.forEach((sub) => {
                      if (sub.partID) { parts.push(sub); } else if (Array.isArray(sub)) {
                        sub.forEach((subsub) => {
                          if (subsub.partID) { parts.push(subsub); }
                        });
                      }
                    });
                  }
              ...
              Meteor.setTimeout(function () {
                imap.addFlags(uid, 'Deleted', Meteor.bindEnvironment(function (err) {
                  ...
                  imap.closeBox(true, (err) => {
                    if (err) { console.log('Failed closing box: ', err); }
                  });
