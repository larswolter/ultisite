...
Template.imageViewer.onCreated(function () {
  ...
  this.autorun(() => {
    const file = UltiSite.Images.findOne(FlowRouter.getParam('_id'));
    if (file) UltiSite.getAnyById(file.associated);
...
Template.imageViewer.events({
  ...
  'click .remove-associated': function (e, t) {
    const userId = t.$(e.currentTarget).attr('data-id');
    console.log();
    const imgId = FlowRouter.getParam('_id');
...
Template.imageViewer.helpers({
  nextImage() {
    ...
    if (assoc) {
      ...
      others.forEach((img, i) => {
        if (FlowRouter.getParam('_id') === img) idx = i;
      });
  ...
  prevImage() {
    ...
    if (assoc) {
      ...
      if (idx !== undefined && idx > 0) return FlowRouter.url('image', { _id: others[idx - 1], associated: assoc });
    }
    return false;
  },
  imageLoadStatus() {
    ...
    if (Template.instance().imageStatus.get() === 'loaded') return undefined;
    return {
