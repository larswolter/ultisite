...
DOMBackend.Teardown = {
  ...
  onElementTeardown(elem, func) {
    let elt = new TeardownCallback(func);

    let propName = DOMBackend.Teardown._CB_PROP;
    if (!elem[propName]) {
  ...
  tearDownElement(elem) {
    ...
    let nodeList = elem.getElementsByTagName('*');
...
$jq.event.special[DOMBackend.Teardown._JQUERY_EVENT_NAME] = {
  ...
  teardown() {
    let elem = this;
    let callbacks = elem[DOMBackend.Teardown._CB_PROP];
    if (callbacks) {
