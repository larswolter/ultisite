...
Template.tweets.helpers({
    ...
    tweets: function () {
        if (FlowRouter.getRouteName() === "tweets") return UltiSite.Tweets.find({}, {
            sort: {
                date: -1
            },
            limit: 30
        });else return UltiSite.Tweets.find({}, {
            sort: {
                date: -1
            },
            limit: 3
...
Template.tweets.events({
    ...
    'submit .tweeter-form': function (e, t) {
        e.preventDefault();
        if (t.$('.tweeter-form .tweet-content').val().length > 160) t.$('.tweeter-form .tweet-content').addClass("bg-danger");
